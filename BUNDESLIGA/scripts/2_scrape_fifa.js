...
(async () => {
    ...
    console.log("🚀 Starting Bundesliga SoFIFA Historical Scraping...");

    for (const [year, rosterId] of Object.entries(ROSTERS)) {
        // Construct URL: Bundesliga ID = 19
        const url = `https://sofifa.com/teams?type=club&lg=19&r=${rosterId}&set=true`;
                ...
                rows.forEach(row => {
                    ...
                    if (nameLink && ratingCell) {
                        ...
                        if (name && !isNaN(rating)) {
                            // Basic mapping to align names slightly directly inside scraping if possible. We will normalize them in python later.
                            data[name] = rating;
                        }
                    }
                });
