...
if idx1 != -1 and idx2 != -1:
            ...
            for idx, m in enumerate(matches):
                ...
                with cols[idx % 2]:
                    ...
                    with st.expander("Ver Análisis Estadístico Detallado"):
                        ...
                        rt_rows = \"\"
                        met_l = [('Elo', 'Elo'), ('FIFA OVR', 'FIFA_Ova'), ('Value', 'Market_Value'), ('xG (L5)', 'xG_Avg_L5'), ('Streak', 'Streak_L5')]
                        for lbl, m_k in met_l:
                            h_v, a_v = X_row.get(f'Home_{m_k}', 0), X_row.get(f'Away_{m_k}', 0)
                            h_s = \"color:#00ff85; font-weight:700;\" if h_v > a_v else \"\"
                            a_s_ = \"color:#00ff85; font-weight:700;\" if a_v > h_v else \"\"
                            rt_rows += f'<div style=\"display:flex; justify-content:space-between; padding:6px 12px; border-bottom:1px solid rgba(255,255,255,0.03); background:rgba(0,0,0,0.1); font-size:11px;\"><div style=\"flex:1; opacity:0.5; font-size:9px;\">{lbl}</div><div style=\"flex:1; text-align:center; {h_s}\">{h_v:,.0f}</div><div style=\"flex:1; text-align:right; {a_s_}\">{a_v:,.0f}</div></div>'
    ...
    # Simple replacement without too much nested escaping
